...
import {  buildMessages, sanitizeAnswer, ChatTurn } from "./prompt";
...
async function generateNode(state: typeof GraphState.State) {
  const messages = buildMessages(state.retrievedContext, state.question, state.history);
  const response = await llm.invoke(messages.map((m) => m.content).join("\n\n"));
